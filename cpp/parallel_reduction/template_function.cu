...
__global__
void reduce_template(int *a, int *s, int size) {
...
void launch_reduce_template_function(int *a, int *s, int size, int grid_dim, int block_dim, cudaStream_t stream) {
    switch (block_dim) {
        case 1024: reduce_template<1024><<<grid_dim, block_dim, 0, stream>>>(a, s, size); break;
        case 512:  reduce_template<512><<<grid_dim, block_dim, 0, stream>>>(a, s, size);  break;
        case 256:  reduce_template<256><<<grid_dim, block_dim, 0, stream>>>(a, s, size);  break;
        case 128:  reduce_template<128><<<grid_dim, block_dim, 0, stream>>>(a, s, size);  break;
        case 64:   reduce_template<64><<<grid_dim, block_dim, 0, stream>>>(a, s, size);   break;
        case 32:   reduce_template<32><<<grid_dim, block_dim, 0, stream>>>(a, s, size);   break;
        default: break;
    }
}
