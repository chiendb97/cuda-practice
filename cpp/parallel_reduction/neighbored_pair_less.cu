...
int main(int argc, char **argv) {
    ...
    CHECK_CUDA_ERROR(cudaStreamSynchronize(stream));
    std::function<void(cudaStream_t)> bound_function_reduce_neighbored_pair_less{
            std::bind(launch_reduce_neighbored_pair_less, d_a, d_s, size, grid_dim, block_dim, stream)};

    float const latency_gpu{measure_performance(bound_function_reduce_neighbored_pair_less, stream, 1, 0)};
