...
void vector_add_cpu(const float *h_input1, const float *h_input2,
                    float *h_output, size_t n) {
  for (int i = 0; i < n; ++i) {
    h_output[i] = h_input1[i] + h_input2[i];
  }
}

__global__ void vector_add(const float4 *d_input1, const float4 *d_input2,
...
void launch_vector_add(const float *d_input1, const float *d_input2,
                       float *d_output, size_t n, int grid_dim, int block_dim,
                       cudaStream_t stream) {
  vector_add<<<grid_dim, block_dim, 0, stream>>>(
...
int main(int argc, char *argv[]) {
  ...
  CHECK_CUDA_ERROR(cudaMemcpyAsync(d_input1, h_input1, n * sizeof(float),
                                   cudaMemcpyHostToDevice, stream));
  CHECK_CUDA_ERROR(cudaMemcpyAsync(d_input2, h_input2, n * sizeof(float),
                                   cudaMemcpyHostToDevice, stream));
  ...
  for (int block_dim = 64; block_dim <= 512; block_dim *= 2) {
    ...
    std::function<void(cudaStream_t)> bound_function_vector_add{
        std::bind(launch_vector_add, d_input1, d_input2, d_output, n, grid_dim,
                  block_dim, stream)};

    float const latency_gpu{
        measure_performance(bound_function_vector_add, stream)};
    std::cout << "Latency for vector add on GPU, block_dim " << block_dim
