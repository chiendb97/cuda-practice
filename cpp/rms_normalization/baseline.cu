...
__global__
void rms_norm(const float4 *__restrict__ X, float4 *Y, size_t B, size_t N, float invN, float eps) {
    auto ti = blockIdx.x;
    auto di = threadIdx.x;
...
template
__global__
void rms_norm<128>(const float4 *__restrict__ X, float4 *Y, size_t B, size_t N, float invN, float eps);

template
__global__
void rms_norm<256>(const float4 *__restrict__ X, float4 *Y, size_t B, size_t N, float invN, float eps);

template
__global__
void rms_norm<512>(const float4 *__restrict__ X, float4 *Y, size_t B, size_t N, float invN, float eps);

template
__global__
void rms_norm<1024>(const float4 *__restrict__ X, float4 *Y, size_t B, size_t N, float invN, float eps);

void launch_rms_norm(const float *d_X, float *d_output, size_t b, size_t n, float eps,
                     int grid_dim,
                     int block_dim, cudaStream_t stream) {
    if (block_dim == 128) {
        rms_norm<128><<<grid_dim, block_dim, 0, stream>>>(reinterpret_cast<const float4 *>(d_X),
                                                          reinterpret_cast<float4 *>(d_output), b, n / 4, 1.0f / n,
                                                          eps);
    } else if (block_dim == 256) {
        rms_norm<256><<<grid_dim, block_dim, 0, stream>>>(reinterpret_cast<const float4 *>(d_X),
                                                          reinterpret_cast<float4 *>(d_output), b, n / 4, 1.0f / n,
                                                          eps);
    } else if (block_dim == 512) {
        rms_norm<512><<<grid_dim, block_dim, 0, stream>>>(reinterpret_cast<const float4 *>(d_X),
                                                          reinterpret_cast<float4 *>(d_output), b, n / 4, 1.0f / n,
                                                          eps);
    } else if (block_dim == 1024) {
        rms_norm<1024><<<grid_dim, block_dim, 0, stream>>>(reinterpret_cast<const float4 *>(d_X),
                                                           reinterpret_cast<float4 *>(d_output), b, n / 4, 1.0f / n,
                                                           eps);
    } else {
...
int main(int argc, char *argv[]) {
    ...
    size_t grid_dim = b;
    std::function<void(cudaStream_t)> bound_function_rms_norm{
        std::bind(launch_rms_norm, d_X, d_output, b, n, eps, grid_dim, block_dim, stream)
    };

    float const latency_gpu{measure_performance(bound_function_rms_norm, stream, num_repeats, num_warmups)};
    std::cout << "Latency for rms norm on GPU, block_dim " << block_dim << ": " << latency_gpu << std::endl;
