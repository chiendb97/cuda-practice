...
template <uint32_t BM, uint32_t BN, uint32_t BK, uint32_t BP, uint32_t NUM_CONSUMER_WARP_GROUPS>
__global__ void
    ...
    if (is_producer)
    {
        ...
        if (threadIdx.x == 0)
        {
            ...
            for (uint32_t block_k = 0; block_k < K / BK; ++block_k)
            {
                ...
                cuda::device::experimental::cp_async_bulk_tensor_2d_global_to_shared(&A_block_smem[p * BM * BK], A_tensor_map, block_k * BK, block_m * BM, full_barrier[p]);
                cuda::device::experimental::cp_async_bulk_tensor_2d_global_to_shared(&B_block_smem[p * BN * BK], B_tensor_map, block_k * BK, block_n * BN, full_barrier[p]);
...
#pragma unroll
        ...
        half *C_block = C + (block_m * BM + consumer_idx * WGMMA_M) * N + block_n * BN;
...
void launch_matmul_hopper_opt2(const uint32_t M, const uint32_t N, const uint32_t K,
                                  const float alpha, half *A, half *B, const float beta, half *C, half *D,
                                  ...
                                  cudaStream_t stream)
{
    ...
    CHECK_CUDA_ERROR(cudaFuncSetAttribute(matrix_multiplication<BM, BN, BK, BP, num_consumner_warp_groups>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));

    matrix_multiplication<BM, BN, BK, BP, num_consumner_warp_groups><<<grid_dim, block_dim, smem_size, stream>>>(M, N, K, alpha, A, B, beta, C, D, A_tensor_map_device, B_tensor_map_device);
}
