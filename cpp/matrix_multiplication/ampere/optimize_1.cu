...
matrix_multiplication(const uint32_t M, const uint32_t N, const uint32_t K,
    ...
    for (uint32_t block_k = 0; block_k < num_block_tiles_k; ++block_k) {
        ...
        tiled_mem_cpy_unrolling<half, BM, BK, NUM_THREADS>(A_block_gmem, A_block_smem, A_stride);
        tiled_mem_cpy_unrolling<half, BK, BN, NUM_THREADS>(B_block_gmem, B_block_smem, B_stride);
...
void launch_matmul_opt1(const uint32_t M, const uint32_t N, const uint32_t K,
                                  const float alpha, half *A, half *B, const float beta, half *C, half *D,
                                  cudaStream_t stream) {
