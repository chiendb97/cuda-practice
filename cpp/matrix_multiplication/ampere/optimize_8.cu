...
template <uint32_t BM, uint32_t BN, uint32_t BK,
          ...
          uint32_t NUM_THREADS, uint32_t STAGES, uint32_t SPLIT_K=1>
__global__ void
...
void launch_matmul_opt8(const uint32_t M, const uint32_t N, const uint32_t K,
                                  const float alpha, half* A, half* B, const float beta, half* C, half* D,
                                  cudaStream_t stream)
{
