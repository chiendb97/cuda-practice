...
matrix_multiplication(const uint32_t M, const uint32_t N, const uint32_t K,
    ...
    for (uint32_t block_k = 0; block_k < num_block_tiles_k; ++block_k) {
        half *A_block_gmem = A + block_m * BM * A_stride + block_k * BK;
        half *B_block_gmem = B + block_k * BK * B_stride + block_n * BN;

        tiled_mem_cpy_unrolling_vectorized<BM, BK, NUM_THREADS, 8>(reinterpret_cast<float4 *>(A_block_gmem), reinterpret_cast<float4 *>(A_block_smem), A_stride);
        tiled_mem_cpy_unrolling_vectorized<BK, BN, NUM_THREADS, 8>(reinterpret_cast<float4 *>(B_block_gmem), reinterpret_cast<float4 *>(B_block_smem), B_stride);
        ...
        for (uint32_t warp_k = 0; warp_k < warp_tiles_per_block_k; ++warp_k) {
            ...
            for (uint32_t mma_m = 0; mma_m < mma_tiles_per_warp_m; ++mma_m) {
                for (uint32_t mma_k = 0; mma_k < mma_tiles_per_warp_k; ++mma_k) {
                    ...
                    const uint32_t thread_offset_bytes =
...
void launch_matmul_opt2(const uint32_t M, const uint32_t N, const uint32_t K,
                                  const float alpha, half *A, half *B, const float beta, half *C, half *D,
                                  cudaStream_t stream) {
    ...
    constexpr uint32_t WARPS_PER_BLOCK_M = 4;
    constexpr uint32_t WARPS_PER_BLOCK_N = 2;
