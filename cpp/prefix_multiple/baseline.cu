...
void prefix_multiple_cpu(const float *X, float *Y, size_t n) {
    Y[0] = X[0];
    for (int i = 1; i < n; ++i) {
        Y[i] = X[i] * Y[i - 1];
    }
...
void launch_prefix_multiple(const float *d_X, float *d_output, int n,
    ...
    CustomMultiple multiple_op;
    void *d_temp_storage = nullptr;
    size_t temp_storage_bytes = 0;
    cub::DeviceScan::InclusiveScan(d_temp_storage, temp_storage_bytes, d_X, d_output, multiple_op, n);
...
int main(int argc, char *argv[]) {
    ...
    prefix_multiple_cpu(h_X, h_target, n);
    std::clock_t time_end = std::clock();

    double latency_cpu = (double) (time_end - time_start) / CLOCKS_PER_SEC * 1000;
    std::cout << "Latency for prefix multiple on CPU: " << latency_cpu << std::endl;
    ...
    std::function<void(cudaStream_t)> bound_function_prefix_multiple{
        std::bind(launch_prefix_multiple, d_X, d_output, n, grid_dim, block_dim, stream)
    };

    float const latency_gpu{measure_performance(bound_function_prefix_multiple, stream, num_repeats, num_warmups)};
    std::cout << "Latency for prefix multiple on GPU, block_dim " << block_dim << ": " << latency_gpu << std::endl;
