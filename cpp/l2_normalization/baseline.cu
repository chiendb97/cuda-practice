...
void l2_norm_cpu(const float *X, float *Y, size_t b, size_t n, float eps) {
    for (int i = 0; i < b; ++i) {
        float sum = 0;
        for (int j = 0; j < n; ++j) {
            sum += X[i * n + j] * X[i * n + j];
        ...
        float inv_rms = 1.0f / sqrt(sum + eps);
...
template
__global__
void l2_norm<128>(const float4 *__restrict__ X, float4 *Y, size_t B, size_t N, float eps);

template
__global__
void l2_norm<256>(const float4 *__restrict__ X, float4 *Y, size_t B, size_t N, float eps);

template
__global__
void l2_norm<512>(const float4 *__restrict__ X, float4 *Y, size_t B, size_t N, float eps);

template
__global__
void l2_norm<1024>(const float4 *__restrict__ X, float4 *Y, size_t B, size_t N, float eps);

void launch_l2_norm(const float *d_X, float *d_output, size_t b, size_t n, float eps,
                    int grid_dim,
                    int block_dim, cudaStream_t stream) {
    if (block_dim == 128) {
        l2_norm<128><<<grid_dim, block_dim, 0, stream>>>(reinterpret_cast<const float4 *>(d_X),
                                                         reinterpret_cast<float4 *>(d_output), b, n / 4, eps);
    } else if (block_dim == 256) {
        l2_norm<256><<<grid_dim, block_dim, 0, stream>>>(reinterpret_cast<const float4 *>(d_X),
                                                         reinterpret_cast<float4 *>(d_output), b, n / 4, eps);
    } else if (block_dim == 512) {
        l2_norm<512><<<grid_dim, block_dim, 0, stream>>>(reinterpret_cast<const float4 *>(d_X),
                                                         reinterpret_cast<float4 *>(d_output), b, n / 4, eps);
    } else if (block_dim == 1024) {
        l2_norm<1024><<<grid_dim, block_dim, 0, stream>>>(reinterpret_cast<const float4 *>(d_X),
                                                          reinterpret_cast<float4 *>(d_output), b, n / 4, eps);
    } else {
...
int main(int argc, char *argv[]) {
    ...
    l2_norm_cpu(h_X, h_target, b, n, eps);
    std::clock_t time_end = std::clock();

    double latency_cpu = (double) (time_end - time_start) / CLOCKS_PER_SEC * 1000;
    std::cout << "Latency for l2 norm on CPU: " << latency_cpu << std::endl;
    ...
    size_t grid_dim = b;
    std::function<void(cudaStream_t)> bound_function_l2_norm{
        std::bind(launch_l2_norm, d_X, d_output, b, n, eps, grid_dim, block_dim, stream)
    };

    float const latency_gpu{measure_performance(bound_function_l2_norm, stream, num_repeats, num_warmups)};
    std::cout << "Latency for l2 norm on GPU, block_dim " << block_dim << ": " << latency_gpu << std::endl;
