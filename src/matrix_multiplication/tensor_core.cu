                                             ...
                                             float beta)
{
    ...
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, T, WMMA_FRAG_LAYOUT_A> a_frag{};
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, T, WMMA_FRAG_LAYOUT_B> b_frag{};
    ...
    for (unsigned k = 0; k < K; k += WMMA_K)
    {
        ...
        T const* matrix_mma_a_mptr{A + matrix_mma_a_row_idx + matrix_mma_a_col_idx * lda};
        T const* matrix_mma_b_mptr{B + matrix_mma_b_row_idx + matrix_mma_b_col_idx * ldb};
    ...
    ACCUM* matrix_mma_c_mptr{C + matrix_mma_c_row_idx + matrix_mma_c_col_idx * ldc};
...
int main(int argc, char** argv)
{
    ...
    std::function<void(cudaStream_t)> bound_function_invoke_matrix_multiplication{
        std::bind(invoke_matrix_multiplication<T, ACCUM>, M, N, K, d_a, d_b, d_c, stream)
    };
